...
3. Bayesian modeling
4. Single-subject hormone modeling with `bayespulse`
...
It is common for hormone elimination to be described by the exponential distribution with decay rate $\lambda_i$, or equivalently, half-life $H_i$.
...
\if{html}{\figure{deconvolution.pdf}{options: width=6in}}
\if{latex}{\figure{deconvolution.pdf}{options: width=6in}}

\figure{'deconvolution.pdf'}

```{r, error = F, warning = F, message = F, fig.align='center', fig.width = 5, fig.height = 5}
...
IF WE CAN FIGURE IT OUT!!!!!!! BC THIS DOESN'T WORK--CAN'T FIGURE OUT HOW TO REDO KAREN'S FIGURE OUT HOW TO DIRECTLY INSERT IT HERE
...
After setting the prior values and starting values, the function `pulse_spec()` can be used to fit the deconvolution model. 

```{r}

results <- bayespulse::fit_pulse(data = simdat$data,
                                 spec = all.spec)
...
```{r, error = F, warning = F, message = F, fig.align='center', fig.width = 6, fig.height = 9}

bayespulse::bp_trace(results)
bayespulse::bp_posteriors(results, type = "histogram")
...
bayespulse::bp_location_posterior(results) 
